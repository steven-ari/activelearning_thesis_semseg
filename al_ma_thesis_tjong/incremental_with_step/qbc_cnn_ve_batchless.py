...
def entropy_dropout_mnist(fcn_model, train_dataset, train_index, device, n_model, dropout_rate, batch_test_size,
    ...
    print('VE of ' + str(np.unique(calc_ve_idx).__len__()) + ' data')
    ...
    with torch.no_grad():
        ...
        for i_batch, (data, target, index) in enumerate(train_dataloader):
            ...
            output_list = output_list_train.argmax(axis=2)
            v_yi_c = np.zeros((output_list_train.shape[1], output_list_train.shape[2]))
            for i_label in range(output_list_train.shape[2]):
                label_stack = np.ones_like(output_list) * i_label
                v_yi_c[:, i_label] = np.sum(output_list == label_stack, axis=0) / output_list.shape[0]
            ve_batch = entropy(v_yi_c, base=2, axis=1)
...
def qbc(n_model, n_train, qbc_batch_size, batch_size, idx_ratio, dataset):
    ...
    # paths
    result_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', dataset, 'incremental_with_step', 'cnn_qbc')
    csv_path = os.path.join(result_path, 'cnn_qbc.csv')
    csv_name_train = 'train.csv'
    csv_name_test = 'test.csv'
    csv_name_index = 'index.csv'
    csv_name_index_compare = 'index_ve.csv'
    mnist_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'data')
    index_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', dataset, 'incremental_with_step', 'cnn_qbc')
    ...
    text = (('VE: n_model: ' + str(n_model)) + (', n_train: ' + str(n_train)) + (', batch_size: ' + str(batch_size))
    ...
    text = ('VE: n_model: ' + str(n_model)) + (', n_train: ' + str(n_train)) + (', n_epoch: ' + str(n_epoch)) + \
    ...
    run_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', dataset, 'incremental_with_step', 'cnn_qbc')

    # write training progress
    csv_path_train = os.path.join(run_path, csv_name_train)
    title = ["VE: Training progress for n_model = " + str(n_model) + ", idx_ratio:  " + str(idx_ratio) + ', for multiple epoch']
    ...
    title = ["VE: Test progress for n_model = " + str(n_model) + ", idx_ratio:  " + str(idx_ratio) + ', for multiple epoch']
    ...
    for i_epoch in range(n_epoch):
        ...
        if len(train_index) == 0:
            ...
            train_index, ve = entropy_dropout_mnist(model, train_dataset, trained_index, device, n_model,
                                                dropout_rate, batch_test_size, i_epoch, n_data=n_train-qbc_batch_size)

        # append with vote entropy
        elif len(train_index) < int(n_train):
            trained_index = np.append(trained_index, train_index[i_epoch*qbc_batch_size:(i_epoch+1)*qbc_batch_size])

            index_text = str(ve[train_index[0]]) + ': ' + str([x for x in train_index]).strip('[]')
