...
def vote_entropy_dropout(fcn_model, train_dataset, train_index, idx_ratio, batch_test_size, device, n_model, dropout_rate, i_epoch, n_data):
    ...
    if idx_ratio[1] < 1.0:
        ...
        print('VE of ' + str(calc_ve_idx.__len__()) + ' data')
        # create dataloader
        ...
        print('VE Dataloader length: ' + str(len(train_dataloader)))
        with torch.no_grad():
            for i_batch, (data, target, index, _) in enumerate(train_dataloader):
                ...
                output_list_train = np.zeros((n_model, data.shape[0],
                                              sample_train.shape[-2], sample_train.shape[-1])).astype(np.int8)
                # fcn_model.to(device)
                # get output of each model
                for i_model in range(n_model):
                    print('Model:' + str(i_model) + ', VE:' + str(i_batch) + '/' + str(len(train_dataloader))
                          + ', epoch: ' + str(i_epoch) + ', total VE: ' + str(len(calc_ve_idx)))
                    print('Shape data:' + str(data.shape))
                    output_list_train[i_model] = fcn_model(data)['out'].detach().cpu().numpy().argmax(axis=1).astype(
                        np.int8)
                    print("Memory allocated:" + str(np.round(torch.cuda.memory_allocated(device) / 1e9, 3)) + ' Gb')
                    print(output_list_train[i_model].mean())
                del data, target
                ve_batch = qbc_preset.vote_entropy_semseg_old(output_list_train, segmen_preset.labels_valid)
    ...
    return indices, ve_train_array, fcn_model
...
def main(dropout):
    ...
    # paths
    save_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', 'random')
    ...
    fcn_model = nn.DataParallel(fcn_model)
    fcn_model.load_state_dict(torch.load(os.path.join(save_path, index_path_name, 'model_weight_epoch_train039.pt')))
    ...
    title = ["Test progress for n_model = " + str(1) + ", idx_ratio:  " + str(idx_ratio)
             + ', for multiple epoch, torch seed: ' + str(None) + 'run_path: ' + run_path +
    ...
    indices, entropy, fcn_model = vote_entropy_dropout(fcn_model, train_dataset_idx, train_index, idx_ratio,
                                                   batch_test_size, device, n_model, dropout_rate, 1,
                                                   n_data=int(n_train))
    # save ranked indices
