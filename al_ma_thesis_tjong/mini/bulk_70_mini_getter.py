    ...
    with torch.no_grad():
        ...
        for i_batch, (img, _, _, img_names) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                output_np = np.argmax(output.detach().cpu().numpy(), axis=1)
...
def train_batch(fcn_model, data_train, target_train, optimizer, device, criterion):
    ...
    # tidy up
    output_cpu, loss_np = output.to('cpu'), loss.item()
    fcn_model.eval()
...
def main(manual_seed_input, source_path_input, dir_name_input, index_path_name):
    """
    ...
    source_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', 've_test')
    csv_name_train = 'train.csv'
    csv_name_test = 'test.csv'
    csv_name_index = 'index.csv'
    dir_name = 'bulk_30_mini_getter_from_vote_90_003_'
    ...
    train_dataset = dataset_preset.Dataset_Cityscapes_n(root=cityscape_path, split='train', mode='fine',
                                                        ...
                                                        n=n_train)
    # read used index
    csv_path_index_source = os.path.join(source_path, index_path_name, csv_name_index)
    ...
    # the models
    fcn_model = smp.Unet('vgg19_bn', classes=segmen_preset.n_labels_valid, activation='softmax')
    ...
    optimizer = torch.optim.Adam([{'params': fcn_model.parameters()}], lr=lr, weight_decay=0.0001)
...
if __name__ == '__main__':
    ...
    main(manual_seed_input=0, source_path_input=source_path_input,
