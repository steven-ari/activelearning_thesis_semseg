...
def entropy_dropout_mnist(fcn_model, train_dataset, train_index, device, n_model, dropout_rate, batch_test_size,
    ...
    with torch.no_grad():
        ...
        for i_batch, (data, target, index) in enumerate(train_dataloader):
            ...
            ve_batch = entropy(np.sum(output_list_train, axis=0), base=2, axis=1)
...
def qbc(n_model, n_train, qbc_batch_size, batch_size, idx_ratio, dataset, seed):
    ...
    # paths
    result_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', dataset, 'only_new_data', 'cnn_qbc')
    ...
    index_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', dataset, 'only_new_data', 'cnn_qbc')
    ...
    run_path = os.path.join(dr(dr(dr(abspath(__file__)))), 'results', dataset, 'only_new_data', 'cnn_qbc')
